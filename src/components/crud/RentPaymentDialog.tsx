...
import { RentPayment, Property, Resident, Booking, getProperties, getResidents, getBookings } from '@/services/api';
...
export default function RentPaymentDialog({ open, initial, onClose, onSave }: Props) {
  ...
  const bookingLabel = (b: Booking) => {
    const name = b.resident?.fullName ?? residents.find(r => r.id === b.residentId)?.fullName ?? b.residentId.slice(0, 8);
    const bed = b.bed ? `Bed #${b.bed.bedNumber}` : '';
    return `${name}${bed ? ' — ' + bed : ''}`;
  };
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers sx={{ p: 2 }}>
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12 }}>
            <TextField select label="Booking" value={form.bookingId} onChange={e => set('bookingId', e.target.value)} fullWidth required size="small">
              {bookings.map(b => <MenuItem key={b.id} value={b.id}>{bookingLabel(b)}</MenuItem>)}
            </TextField>
          </Grid>
